...
function HelmOutput({
  ...
  return (
    <div className="w-full flex flex-col bg-white p-6 rounded-lg shadow-lg text-black space-y-4">
      ...
      {/* Error Alert */}
      <ErrorAlert message={backendError} showDetailsButton={!!(stderr || stdout)} />
      ...
      <DownloadButton
        ...
        outputFormat={outputFormat}
        filename={filename}
        filenameFallback={`helm-diagram.${(outputFormat || 'png').toLowerCase()}`}
      />
      ...
      {/* Notation Options */}
      {diagram && <NotationOptions diagramType="helm" />}
    </div>
  );
}

export default HelmOutput;
